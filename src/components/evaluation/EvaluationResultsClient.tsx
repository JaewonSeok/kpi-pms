...
function GrowthRecommendationSection({ viewModel }: { viewModel: EvaluationResultViewModel }) { return <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]"><div className="space-y-6"><SectionCard title="상위 강점 3개" description="다음 반기에도 계속 활용할 수 있는 강점입니다."><ListPanel tone="positive" items={viewModel.growth.strengths} /></SectionCard><SectionCard title="보완 필요 역량 3개" description="리더와 우선순위를 맞춰서 보완할 영역입니다."><ListPanel tone="attention" items={viewModel.growth.improvements} /></SectionCard><SectionCard title="추천 액션 아이템" description="바로 실행 가능한 다음 행동을 제안합니다."><ListPanel tone="neutral" items={viewModel.growth.actions} /></SectionCard></div><div className="space-y-6"><SectionCard title="리더와 1:1에서 논의할 질문" description="다음 체크인과 성장 계획 대화에 바로 활용할 수 있습니다."><ListPanel tone="neutral" items={viewModel.growth.discussionQuestions} /></SectionCard><ActionLink icon={<Sparkles className="h-4 w-4" />} label="AI 보조 작성으로 이어보기" href="/evaluation/assistant" description="강점과 보완 포인트를 바탕으로 성장 계획 초안과 평가 보조 작성을 이어갈 수 있습니다." /></div></div> }
...
function RelatedActionLinks() {
  const links = [{ href: '/evaluation/appeal', label: '이의 신청' }, { href: '/evaluation/assistant', label: 'AI 보조 작성' }, { href: '/evaluation/360/results', label: '360 다면평가' }, { href: '/kpi/personal', label: '개인 KPI' }, { href: '/kpi/monthly', label: '월간 실적' }, { href: '/checkin', label: '체크인 일정' }, { href: '/compensation/my', label: '내 보상 결과' }]
