...
function EvidenceSection({ viewModel }: { viewModel: EvaluationResultViewModel }) { return <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]"><div className="space-y-6"><SectionCard title="연결된 개인 KPI 요약" description="조직 목표와 연결된 개인 KPI의 최신 실적입니다."><div className="space-y-3">{viewModel.evidence.kpis.map((kpi) => <div key={kpi.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="flex items-center justify-between gap-3"><div className="font-semibold text-slate-900">{kpi.title}</div><InfoPill label={kpi.status ?? '상태 없음'} /></div><div className="mt-2 text-sm text-slate-600">목표 {formatMetric(kpi.target)} / 실적 {formatMetric(kpi.actual)} / 달성률 {kpi.achievementRate !== undefined ? `${kpi.achievementRate.toFixed(1)}%` : '미집계'}</div></div>)}</div></SectionCard><SectionCard title="월간 실적 / 체크인 / 피드백" description="평가 결과에 가장 가까운 실행 근거를 최근 순서대로 보여줍니다."><div className="space-y-4"><SubBlock title="월간 실적 요약">{viewModel.evidence.monthlyRecords.map((record) => <TimelineItem key={record.month} title={record.month} description={record.comment || '월간 코멘트 없음'} meta={record.achievementRate !== undefined ? `${record.achievementRate.toFixed(1)}%` : '미집계'} />)}</SubBlock><SubBlock title="체크인 / 1:1 요약">{viewModel.evidence.checkins.map((checkin, index) => <TimelineItem key={`${checkin.date}-${index}`} title={formatDateTime(checkin.date)} description={checkin.summary} meta={`${checkin.type} / ${checkin.status}`} />)}</SubBlock><SubBlock title="피드백 / 코멘트">{viewModel.evidence.feedbacks.map((feedback, index) => <TimelineItem key={`${feedback.date}-${index}`} title={`${feedback.author} · ${formatDateTime(feedback.date)}`} description={feedback.content} />)}</SubBlock></div></SectionCard></div><div className="space-y-6"><SectionCard title="이번 결과에 가장 큰 영향을 준 근거" description="핵심 근거 3개를 하이라이트로 정리했습니다."><div className="space-y-3">{viewModel.evidence.highlights.map((highlight) => <HighlightCard key={highlight.title} title={highlight.title} summary={highlight.summary} tone={highlight.tone} />)}</div></SectionCard><SectionCard title="첨부 / 증빙" description="평가 결과 설명에 활용된 증빙 목록입니다."><div className="space-y-3">{viewModel.evidence.attachments.length ? viewModel.evidence.attachments.map((attachment, index) => <TimelineItem key={`${attachment.label}-${index}`} title={attachment.label} description={attachment.source} />) : <EmptyCard title="연결된 첨부가 없습니다." description="월간 실적 또는 평가 근거에 등록된 첨부가 있으면 여기에 표시됩니다." />}</div></SectionCard></div></div> }

function CalibrationTimelineSection({ viewModel }: { viewModel: EvaluationResultViewModel }) { return <div className="space-y-6"><SectionCard title="조정 전 / 조정 후 비교" description="캘리브레이션이 있었다면 등급 변화와 사유를 비교해서 보여줍니다."><div className="grid gap-4 md:grid-cols-3">{viewModel.calibration.draftGrade !== undefined ? <SummaryCard icon={<ClipboardList className="h-5 w-5" />} label="초안 등급" value={viewModel.calibration.draftGrade} description="조정 전 기준" /> : null}<SummaryCard icon={<ShieldCheck className="h-5 w-5" />} label="최종 등급" value={viewModel.calibration.finalGrade ?? '없음'} description="최종 확정 결과" />{viewModel.calibration.draftGrade !== undefined ? <SummaryCard icon={<Layers3 className="h-5 w-5" />} label="조정 상태" value={viewModel.calibration.adjusted ? '조정 반영' : '조정 없음'} description={viewModel.calibration.reason ?? '사유 없음'} /> : null}</div></SectionCard><SectionCard title="공개 / 확정 이력" description="감사 가능한 형태로 상태 변경 이력을 남깁니다."><div className="space-y-4">{viewModel.calibration.logs.map((log, index) => <div key={`${log.date}-${index}`} className="flex gap-4"><div className="mt-1 flex h-9 w-9 items-center justify-center rounded-full bg-slate-900 text-white">{index + 1}</div><div className="flex-1 rounded-2xl border border-slate-200 bg-white p-4"><div className="flex flex-col gap-1 md:flex-row md:items-center md:justify-between"><div className="font-semibold text-slate-900">{log.action}</div><div className="text-xs text-slate-500">{formatDateTime(log.date)}</div></div><div className="mt-1 text-sm text-slate-500">{log.actor}</div>{log.detail ? <p className="mt-2 text-sm leading-6 text-slate-700">{log.detail}</p> : null}</div></div>)}</div></SectionCard></div> }

function GrowthRecommendationSection({ viewModel }: { viewModel: EvaluationResultViewModel }) { return <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]"><div className="space-y-6"><SectionCard title="상위 강점 3개" description="다음 반기에도 계속 활용할 수 있는 강점입니다."><ListPanel tone="positive" items={viewModel.growth.strengths} /></SectionCard><SectionCard title="보완 필요 역량 3개" description="평가권자와 우선순위를 맞춰서 보완할 영역입니다."><ListPanel tone="attention" items={viewModel.growth.improvements} /></SectionCard><SectionCard title="추천 액션 아이템" description="바로 실행 가능한 다음 행동을 제안합니다."><ListPanel tone="neutral" items={viewModel.growth.actions} /></SectionCard></div><div className="space-y-6"><SectionCard title="평가권자와 1:1에서 논의할 질문" description="다음 체크인과 성장 계획 대화에 바로 활용할 수 있습니다."><ListPanel tone="neutral" items={viewModel.growth.discussionQuestions} /></SectionCard><ActionLink icon={<Sparkles className="h-4 w-4" />} label="AI 보조 작성으로 이어보기" href="/evaluation/workbench" description="강점과 보완 포인트를 바탕으로 성장 계획 초안과 평가 보조 작성을 이어갈 수 있습니다." /></div></div> }

function DetailPanel({ item }: { item: DetailItem }) { if (!item) { return <SectionCard title="항목 상세" description="점수 항목을 선택하면 상세 코멘트와 근거를 볼 수 있습니다."><EmptyCard title="선택된 항목이 없습니다." description="세부 점수 탭에서 항목을 선택해 주세요." /></SectionCard> } return <SectionCard title="항목 상세" description="평가 단계별 점수와 최종 코멘트를 함께 확인합니다."><div className="space-y-4"><InfoGridCard label="항목명" value={`${item.group} · ${item.title}`} /><div className="grid gap-3 md:grid-cols-2"><MiniStat label="자기평가" value={item.selfScore !== undefined ? `${item.selfScore.toFixed(1)}점` : '없음'} />{item.managerScore !== undefined ? <MiniStat label="1차 평가" value={`${item.managerScore.toFixed(1)}점`} /> : null}{item.reviewerScore !== undefined ? <MiniStat label="2차 평가" value={`${item.reviewerScore.toFixed(1)}점`} /> : null}{item.finalScore !== undefined ? <MiniStat label="최종값" value={`${item.finalScore.toFixed(1)}점`} /> : null}</div><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm leading-6 text-slate-700">{item.comment || '등록된 상세 코멘트가 없습니다.'}</div></div></SectionCard> }
    ...
    >
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          {item.finalScore !== undefined ? <InfoPill label={`최종 ${item.finalScore.toFixed(1)}점`} /> : null}
          <InfoPill label={`자기 ${item.selfScore?.toFixed(1) ?? '-'}점`} />
          {item.deltaFromSelf !== undefined ? <InfoPill label={`차이 ${item.deltaFromSelf > 0 ? '+' : ''}${item.deltaFromSelf.toFixed(1)}`} /> : null}
        </div>
      </div>
    </button>
  )
