    ...
    <div className="space-y-6">
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-blue-500">Executive Statistics</p>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">평가 주기와 조직 범위를 바꾸면 모든 섹션이 같은 기준으로 다시 계산됩니다.</p>
          </div>
      ...
      {data.state !== 'ready' || !data.sections ? (
        <StateCard
          title={data.state === 'permission-denied' ? '접근 권한이 없습니다.' : '표시할 통계가 없습니다.'}
          description={data.message ?? '선택한 조건에 맞는 데이터를 찾지 못했습니다.'}
        />
      ) : (
        <>
          <section id="overview-section" className="space-y-4">
            <SectionHeader
              ...
              title="경영 한눈 보기"
              description="대표이사가 가장 먼저 확인해야 할 핵심 지표만 요약했습니다."
            />
          ...
          <div className="grid gap-6 xl:grid-cols-2">
            ...
            >
              {data.sections.evaluationOperations.stageStatus.length ? (
                <StatisticsStageStatusChart data={data.sections.evaluationOperations.stageStatus} />
              ) : (
                <EmptyState icon={<Users className="h-6 w-6" />} message="표시할 평가 단계 데이터가 없습니다." />
              )}
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                {data.sections.evaluationOperations.exceptionCards.map((card) => (
                  <MetricCard key={card.label} card={card} compact />
                ))}
              </div>
              <SectionTable
                headers={['조직', '대상 인원', '진행률', '반려', '지연', '최종 확정']}
                rows={data.sections.evaluationOperations.departmentRows.map((row) => [
            ...
            >
              {data.sections.performanceDistribution.companyDistribution.length ? (
                <StatisticsDistributionChart
                  data={data.sections.performanceDistribution.companyDistribution.map((item) => ({
                    label: item.grade,
                    ratio: item.ratio,
                    count: item.count,
                  }))}
                ...
                <EmptyState icon={<LineChart className="h-6 w-6" />} message="표시할 평가 분포가 없습니다." />
              )}
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                {data.sections.performanceDistribution.cards.map((card) => (
                  <MetricCard key={card.label} card={card} compact />
                ))}
              </div>
              <div className="mt-5">
                ...
                <div className="mt-3">
                  {data.sections.performanceDistribution.beforeAfterDistribution.length ? (
                    <StatisticsComparisonChart
                      data={data.sections.performanceDistribution.beforeAfterDistribution.map((item) => ({
                        label: item.grade,
                        first: item.before,
                        second: item.after,
                      }))}
              ...
              {data.sections.performanceDistribution.notice ? (
              ...
              <SectionTable
                ...
                rows={data.sections.performanceDistribution.departmentRows.map((row) => [
          ...
          <div className="grid gap-6 xl:grid-cols-2">
            ...
            >
              {data.sections.kpiExecution.trend.length ? (
                <StatisticsTrendChart data={data.sections.kpiExecution.trend} />
              ) : (
                <EmptyState icon={<LineChart className="h-6 w-6" />} message="추세를 그릴 월간 KPI 데이터가 없습니다." />
              )}
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                {data.sections.kpiExecution.cards.map((card) => (
                  <MetricCard key={card.label} card={card} compact />
                ))}
              </div>
              <SectionTable
                headers={['조직', '인원', '개인 목표 설정률', '정렬률', '체크인 완료율', '평균 진행률', '리스크']}
                rows={data.sections.kpiExecution.departmentRows.map((row) => [
                  <SmartLink key={`${row.departmentId}-name`} href={row.href} className="font-semibold text-slate-900 hover:text-blue-600">
                    {row.departmentName}
                  </SmartLink>,
            ...
            <Panel
              id="risk-section"
              title="조직 건강 / 리스크"
              description="실적 저하, 증빙 부족, 반려, 이의제기처럼 바로 개입해야 할 위험 신호를 묶었습니다."
            >
              <div className="grid gap-3 md:grid-cols-2">
                {data.sections.organizationRisk.cards.map((card) => (
                  <MetricCard key={card.label} card={card} compact />
                ))}
              </div>
              <div className="mt-5">
                {data.sections.organizationRisk.departmentRows.length ? (
                  <StatisticsHorizontalBarChart
                    data={data.sections.organizationRisk.departmentRows.slice(0, 6).map((row) => ({
              ...
              <SectionTable
                ...
                rows={data.sections.organizationRisk.departmentRows.map((row) => [
                  <SmartLink key={`${row.departmentId}-name`} href={row.href} className="font-semibold text-slate-900 hover:text-blue-600">
                    {row.departmentName}
                  </SmartLink>,
          ...
          <div className="grid gap-6 xl:grid-cols-2">
            ...
            >
              <p className="rounded-2xl bg-slate-50 px-4 py-3 text-sm text-slate-600">
                {data.sections.readinessProxy.notice}
              </p>
              {data.sections.readinessProxy.state === 'empty' ? (
                <div className="mt-4">
                  <EmptyState icon={<BriefcaseBusiness className="h-6 w-6" />} message={data.sections.readinessProxy.message ?? '준비도 데이터를 표시할 수 없습니다.'} />
                </div>
              ) : (
                <>
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    {data.sections.readinessProxy.cards.map((card) => (
                      <MetricCard key={card.label} card={card} compact />
                    ))}
                  </div>
                  <div className="mt-5">
                    {data.sections.readinessProxy.trackDistribution.length ? (
                      <StatisticsHorizontalBarChart
                        data={data.sections.readinessProxy.trackDistribution.map((item) => ({
                          label: item.label,
                  ...
                  <SectionTable
                    headers={['조직', '평균 점수', '인원']}
                    rows={data.sections.readinessProxy.departmentRows.map((row) => [
                      row.departmentName,
                      `${row.averageScore}점`,
                      String(row.count),
                    ])}
                  />
                  {data.sections.readinessProxy.feederPool.length ? (
                    <div className="mt-4 flex flex-wrap gap-2">
                      {data.sections.readinessProxy.feederPool.map((row) => (
            ...
            <Panel
              id="fairness-section"
              title="공정성 / 보정 필요 신호"
              description="보정, 정합성, 평가 품질 경고를 함께 보며 리뷰 리스크를 확인합니다."
            >
              <div className="grid gap-3 md:grid-cols-2">
                {data.sections.fairness.cards.map((card) => (
                  <MetricCard key={card.label} card={card} compact />
                ))}
              </div>
              <div className="mt-5">
                {data.sections.fairness.alignmentDistribution.some((item) => item.count > 0) ? (
                  <StatisticsDistributionChart
                    data={data.sections.fairness.alignmentDistribution.map((item) => ({
                      label: item.label,
                      ratio: item.count,
                    }))}
                    leftLabel="건수"
                  ...
                  <EmptyState icon={<Building2 className="h-6 w-6" />} message={data.sections.fairness.coverageLabel} />
                )}
              </div>
              <p className="mt-4 rounded-2xl bg-slate-50 px-4 py-3 text-sm text-slate-600">
                {data.sections.fairness.coverageLabel}
              </p>
              <SectionTable
                headers={['품질 신호', '건수']}
                rows={data.sections.fairness.qualityWarnings.map((item) => [item.label, String(item.count)])}
              />
              <div className="mt-4">
                <SmartLink
                  href={data.sections.fairness.detailHref}
