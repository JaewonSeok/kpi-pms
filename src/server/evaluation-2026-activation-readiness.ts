...
import {
  buildEvaluation2026DryRunRehearsalGuardrails,
  type Evaluation2026DryRunRehearsalGuardrails,
} from '@/server/evaluation-2026-dryrun-rehearsal-guardrails'
import {
  buildEvaluation2026BackfillDryRunCommandRunbook,
  type Evaluation2026BackfillDryRunCommandRunbook,
} from '@/server/evaluation-2026-backfill-dryrun-command-runbook'
import {
  buildEvaluation2026DryRunGoNoGoFreezePack,
  type Evaluation2026DryRunGoNoGoFreezePack,
} from '@/server/evaluation-2026-dryrun-go-no-go-freeze'
import {
  buildEvaluation2026DryRunUnlockPlan,
  type Evaluation2026DryRunUnlockPlan,
} from '@/server/evaluation-2026-dryrun-unlock-plan'
...
export type Evaluation2026ActivationReadinessResult = {
  ...
  dryRunGoNoGoFreezePack: Evaluation2026DryRunGoNoGoFreezePack
  dryRunUnlockPlan: Evaluation2026DryRunUnlockPlan
...
export async function getEvaluation2026ActivationReadiness(params: {
  ...
  const readinessScenarioSimulator = buildEvaluation2026ReadinessScenarioSimulator({
    integratedReadinessSnapshot,
    readinessActionPlan,
  ...
  const backfillDryRunPreflightPack = buildEvaluation2026BackfillDryRunPreflightPack({
    integratedReadinessSnapshot,
    readinessExecutionBoard,
  ...
  const dryRunRehearsalGuardrails = buildEvaluation2026DryRunRehearsalGuardrails({
    integratedReadinessSnapshot,
    backfillDryRunPreflightPack,
    dryRunOutputReviewTemplate,
  })
  const backfillDryRunCommandRunbook = buildEvaluation2026BackfillDryRunCommandRunbook({
    integratedReadinessSnapshot,
    backfillDryRunPreflightPack,
    dryRunOutputReviewTemplate,
    dryRunRehearsalGuardrails,
  })
  const dryRunGoNoGoFreezePack = buildEvaluation2026DryRunGoNoGoFreezePack({
    integratedReadinessSnapshot,
    backfillDryRunPreflightPack,
    dryRunOutputReviewTemplate,
    dryRunRehearsalGuardrails,
  ...
  const dryRunUnlockPlan = buildEvaluation2026DryRunUnlockPlan({
