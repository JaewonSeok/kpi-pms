...
async function main() {
  ...
  await run('employee page route and client use the new gate loader and Korean status copy', () => {
    ...
    assert.equal(clientSource.includes('AI 활용 제출'), true)
    assert.equal(clientSource.includes('AI 역량평가'), false)
    assert.equal(clientSource.includes('업무에서 활용한 AI 사례'), true)
    assert.equal(clientSource.includes('공식 평가 점수나 등급을 자동 산정하지 않습니다'), true)
    assert.equal(clientSource.includes('Evaluation.totalScore / gradeId 저장 없음'), true)
    assert.equal(clientSource.includes('임시저장'), true)
    assert.equal(clientSource.includes('제출 상태'), true)
    assert.equal(clientSource.includes('증빙'), true)
    assert.equal(clientSource.includes('AI가 평가 점수를 산정'), false)
    assert.equal(clientSource.includes('AI 등급 산정'), false)
    assert.equal(clientSource.includes('자동 등급 반영'), false)
    assert.equal(clientSource.includes('공식 점수 반영'), false)
