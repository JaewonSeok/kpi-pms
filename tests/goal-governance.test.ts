...
async function main() {
  ...
  await run('org KPI loader and client reflect read-only gating, team approval state, and tag context in the live workspace', () => {
    ...
    assert.equal(loaderSource.includes("label: '실 KPI'"), true)
    ...
    assert.equal(clientSource.includes('xl:grid-cols-[minmax(0,1fr)_560px]'), true)
    assert.equal(clientSource.includes('mt-5 border-t border-slate-200 pt-4'), true)
    assert.equal(clientSource.includes('ActionButton label="AI 개선"'), false)
    ...
    assert.equal(loaderSource.includes('riskCount: number'), false)
    ...
    assert.equal(clientSource.includes('연결 리스크'), false)
    assert.equal(clientSource.includes('연결 위험 신호'), false)
