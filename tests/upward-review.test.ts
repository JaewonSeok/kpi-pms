...
async function main() {
  ...
  await run('leadership response page source includes list, draft, submit, and read-only flow', () => {
    ...
    assert.equal(client.includes('문항 카테고리'), true)
    assert.equal(client.includes('제출 요약'), true)
    assert.equal(client.includes('공식 평가 점수/등급을 자동 산정하지 않습니다'), true)
  ...
  await run('leadership results page exposes AI leadership coaching without schema changes', () => {
    const client = read('src/components/evaluation/upward/UpwardReviewWorkspaceClient.tsx')
    ...
    assert.equal(panel.includes('/api/feedback/upward/results/ai-coaching'), true)
    assert.equal(client.includes('LeadershipDiagnosisAiCoachingPanel'), true)
    assert.equal(panel.includes('30일 실행 계획'), true)
    assert.equal(panel.includes('60일 실행 계획'), true)
    assert.equal(panel.includes('90일 실행 계획'), true)
    assert.equal(panel.includes('성장 대화 질문'), true)
    assert.equal(panel.includes('팀장/HR 참고용 코칭 가이드'), true)
    ...
    assert.equal(prompt.includes('공식 평가 점수, 등급, 보상, 승진, 인사 판단을 산정하지 않는다.'), true)
    assert.equal(prompt.includes('응답자/평가자의 신원, 이름, 이메일, 사번'), true)
    assert.equal(prompt.includes('정해진 JSON schema에 맞춰 응답하세요.'), true)
    assert.equal(aiLib.includes('UpwardReviewAICoachingResultSchema'), true)
    assert.equal(aiLib.includes('actionPlan30Days'), true)
    assert.equal(aiLib.includes('managerHrGuide'), true)
  })
  ...
  await run('results page source includes visibility guidance and audit logging', () => {
    ...
    assert.equal(client.includes('개별 평가자 정보는 공개되지 않습니다.'), true)
    assert.equal(client.includes('공식 평가 점수/등급 미산정'), true)
    assert.equal(client.includes('카테고리별 응답 요약'), true)
    assert.equal(client.includes('강점 Top 3'), true)
    assert.equal(client.includes('보완 Top 3'), true)
    assert.equal(client.includes('리더십 행동 요약'), true)
    assert.equal(client.includes('다음 체크인 질문'), true)
    assert.equal(client.includes('성장 액션 / 후속 액션'), true)
    assert.equal(client.includes('PDF 다운로드'), false)
    assert.equal(client.includes('결과 비공개'), true)
    assert.equal(client.includes('강점 Top 3 / 보완 Top 3'), true)
    assert.equal(client.includes('성장 액션 / 후속 액션'), true)
