...
async function main() {
  ...
  await run('activation readiness API and UI are read-only and admin-gated', () => {
    ...
    const sandboxRequirementsSource = read('src/lib/evaluation-2026-sandbox-draft-requirements.ts')
    ...
    assert.equal(clientSource.includes('2026 Backfill Dry-run Preflight Pack'), true)
    assert.equal(clientSource.includes('backfill dry-run 검토를 준비하기 위한 읽기 전용 preflight'), true)
    assert.equal(clientSource.includes('Command templates are text only'), true)
    assert.equal(clientSource.includes('Dry-run command reference'), true)
    assert.equal(clientSource.includes('Expected dry-run output checklist'), true)
    assert.equal(clientSource.includes('DB backup checklist'), true)
    ...
    assert.equal(clientSource.includes('Production apply command must not be placed in UI'), true)
    assert.equal(clientSource.includes('2026 Dry-run Output Review Template'), true)
    assert.equal(clientSource.includes('향후 dry-run 결과를 검토하기 위한 읽기 전용 템플릿'), true)
    assert.equal(clientSource.includes('local-only paste helper'), true)
    assert.equal(clientSource.includes('Local-only paste helper'), true)
    ...
    assert.equal(clientSource.includes('uploadAvailable'), true)
    assert.equal(clientSource.includes('Expected output fields'), true)
    assert.equal(clientSource.includes('Must-pass criteria'), true)
    assert.equal(clientSource.includes('Red flags'), true)
    assert.equal(clientSource.includes('Decision outcomes'), true)
    assert.equal(clientSource.includes('Next action mapping'), true)
    assert.equal(clientSource.includes('2026 Dry-run Rehearsal &amp; Guardrails'), true)
    assert.equal(clientSource.includes('dry-run 결과 판독과 apply 가드레일을 사전 리허설하기 위한 읽기 전용 화면'), true)
    assert.equal(clientSource.includes('Script surface inventory'), true)
    assert.equal(clientSource.includes('Apply guardrail status'), true)
    assert.equal(clientSource.includes('Fixture rehearsal'), true)
    assert.equal(clientSource.includes('Local-only paste validator'), true)
    assert.equal(clientSource.includes('local-only classification'), true)
    assert.equal(clientSource.includes('2026 Backfill Dry-run Command Runbook'), true)
    assert.equal(clientSource.includes('향후 dry-run-only 실행 절차를 문서화합니다'), true)
    assert.equal(clientSource.includes('Operator summary'), true)
    assert.equal(clientSource.includes('Pre-run checklist'), true)
    assert.equal(clientSource.includes('Dry-run-only command reference'), true)
    assert.equal(clientSource.includes('Apply command warning'), true)
    assert.equal(clientSource.includes('Output archive checklist'), true)
    assert.equal(clientSource.includes('Log watch checklist'), true)
    assert.equal(clientSource.includes('Abort conditions'), true)
    assert.equal(clientSource.includes('Handoff checklist'), true)
    ...
    assert.equal(clientSource.includes('Required evidence pack'), true)
    assert.equal(clientSource.includes('HR unlock actions'), true)
    assert.equal(clientSource.includes('Developer unlock actions'), true)
    assert.equal(clientSource.includes('Sign-off checklist'), true)
    assert.equal(clientSource.includes('Next checkpoint'), true)
    assert.equal(clientSource.includes('Freeze Pack exports'), true)
    assert.equal(clientSource.includes('Freeze Pack copy actions'), true)
    assert.equal(clientSource.includes('Export Markdown'), true)
    assert.equal(clientSource.includes('Export TSV'), true)
    ...
    assert.equal(clientSource.includes('2026 Interactive Pilot Walkthrough'), true)
    assert.equal(clientSource.includes('local-only'), true)
    assert.equal(clientSource.includes('대상자'), true)
    assert.equal(clientSource.includes('KPI 항목'), true)
    assert.equal(clientSource.includes('자기평가 preview'), true)
    assert.equal(clientSource.includes('1차 평가 preview'), true)
    assert.equal(clientSource.includes('2차/최종 평가 preview'), true)
    assert.equal(clientSource.includes('대표이사 조정 preview'), true)
    assert.equal(clientSource.includes('pilot completion'), true)
    assert.equal(clientSource.includes('local score preview'), true)
    assert.equal(clientSource.includes('local grade preview'), true)
    assert.equal(clientSource.includes('KPI item preview'), true)
    assert.equal(clientSource.includes('result summary'), true)
    assert.equal(clientSource.includes('first reviewer comment'), true)
    assert.equal(clientSource.includes('second/final reviewer comment'), true)
    assert.equal(clientSource.includes('CEO adjustment preview'), true)
    assert.equal(clientSource.includes('reason required if CEO adjustment is not zero'), true)
    assert.equal(clientSource.includes('no API write calls'), true)
    assert.equal(clientSource.includes('Preview summary 보기'), true)
    assert.equal(clientSource.includes('Self evaluation preview 보기'), true)
    assert.equal(clientSource.includes('First review preview 보기'), true)
    assert.equal(clientSource.includes('Final review preview 보기'), true)
    assert.equal(clientSource.includes('Score/grade preview 보기'), true)
    assert.equal(clientSource.includes('Safety summary 보기'), true)
    ...
    assert.equal(clientSource.includes('공식 점수, 공식 등급, totalScore, gradeId, backfill은 실행하지 않습니다'), true)
    assert.equal(clientSource.includes('2026 Evaluation Workbench Pilot Alignment'), true)
    assert.equal(clientSource.includes('workbench preview'), true)
    assert.equal(clientSource.includes('2026 Evaluation Workbench Pilot Shell'), true)
    assert.equal(clientSource.includes('/evaluation/workbench renders this preview-only pilot shell'), true)
    assert.equal(clientSource.includes('전용 Workbench Pilot 열기'), true)
    assert.equal(clientSource.includes('presentationMode="workbench-pilot"'), true)
    assert.equal(clientSource.includes('DedicatedWorkbenchPilotRoute2026'), true)
    assert.equal(clientSource.includes('pilot target employee'), true)
    assert.equal(clientSource.includes('local achievement level'), true)
    assert.equal(clientSource.includes('SELF 자기평가'), true)
    assert.equal(clientSource.includes('FIRST 1차 평가'), true)
    assert.equal(clientSource.includes('SECOND 2차 평가'), true)
    assert.equal(clientSource.includes('FINAL 최종 평가'), true)
    assert.equal(clientSource.includes('CEO_ADJUST 대표이사 조정'), true)
    assert.equal(clientSource.includes('SCORE_PREVIEW'), true)
    assert.equal(clientSource.includes('GRADE_PREVIEW'), true)
    assert.equal(clientSource.includes('first reviewer comment'), true)
    assert.equal(clientSource.includes('second/final reviewer comment'), true)
    assert.equal(clientSource.includes('CEO adjustment amount'), true)
    assert.equal(clientSource.includes('score preview integration'), true)
    assert.equal(clientSource.includes('grade preview integration'), true)
    assert.equal(clientSource.includes('safety panel'), true)
    assert.equal(clientSource.includes('API write calls false'), true)
    assert.equal(clientSource.includes('Workbench preview summary 보기'), true)
    assert.equal(clientSource.includes('SELF item preview 보기'), true)
    assert.equal(clientSource.includes('CEO adjustment item preview 보기'), true)
    ...
    assert.equal(clientSource.includes('Workbench item-level evaluation table'), true)
    assert.equal(clientSource.includes('Selected KPI item detail panel'), true)
    assert.equal(clientSource.includes('SELF item-level preview'), true)
    assert.equal(clientSource.includes('FIRST reviewer item-level preview'), true)
    assert.equal(clientSource.includes('SECOND/FINAL item-level preview'), true)
    assert.equal(clientSource.includes('CEO_ADJUST item-level preview'), true)
    assert.equal(clientSource.includes('Score/grade side panel'), true)
    assert.equal(clientSource.includes('Stage handoff summary'), true)
    assert.equal(clientSource.includes('Workbench item table 보기'), true)
    assert.equal(clientSource.includes('Selected KPI item preview 보기'), true)
    assert.equal(clientSource.includes('SELF item preview 보기'), true)
    assert.equal(clientSource.includes('FIRST review item preview 보기'), true)
    assert.equal(clientSource.includes('SECOND/FINAL item preview 보기'), true)
    assert.equal(clientSource.includes('CEO adjustment item preview 보기'), true)
    assert.equal(clientSource.includes('Score/grade side panel 보기'), true)
    assert.equal(clientSource.includes('Stage handoff summary 보기'), true)
    assert.equal(clientSource.includes('KPI title'), true)
    assert.equal(clientSource.includes('local calculated score'), true)
    assert.equal(clientSource.includes('adjustment reason required if adjustment is not zero'), true)
    assert.equal(clientSource.includes('EvaluationItem creation false'), true)
    assert.equal(clientSource.includes('2026 Evaluation Sandbox Draft Bridge'), true)
    assert.equal(sandboxRequirementsSource.includes('Sandbox draft 저장은 아직 지원하지 않습니다. 전용 스키마 설계가 필요합니다.'), true)
    assert.equal(clientSource.includes('No pilot-draft API route is exposed.'), true)
    assert.equal(clientSource.includes('No Sandbox draft load/save/reset controls are rendered.'), true)
    assert.equal(clientSource.includes('safe without migration'), true)
    assert.equal(sandboxRequirementsSource.includes('EVALUATION_2026_SANDBOX_DRAFT_MODE'), true)
    assert.equal(sandboxRequirementsSource.includes('SANDBOX_2026_WORKBENCH_PILOT'), true)
    assert.equal(sandboxRequirementsSource.includes("status: 'SCHEMA_REQUIRED'"), true)
    assert.equal(sandboxRequirementsSource.includes('Evaluation.isDraft는 공식 평가 초안 상태로 이미 사용됩니다.'), true)
    assert.equal(sandboxRequirementsSource.includes('Evaluation unique key는 evalCycleId + targetId + evalStage'), true)
    assert.equal(sandboxRequirementsSource.includes('EvaluationSandboxSession'), true)
    assert.equal(sandboxRequirementsSource.includes('EvaluationSandboxItem'), true)
    assert.equal(sandboxRequirementsSource.includes('sandbox Evaluation row creation'), true)
    assert.equal(sandboxRequirementsSource.includes('totalScore persistence'), true)
    assert.equal(sandboxRequirementsSource.includes('gradeId persistence'), true)
    assert.equal(clientSource.includes('/api/evaluation/workbench/pilot-draft'), false)
    assert.equal(clientSource.includes('Sandbox draft 불러오기'), false)
    assert.equal(clientSource.includes('Sandbox draft 초기화'), false)
    ...
    assert.equal(workbenchPageSource.includes('presentationMode="workbench-pilot"'), true)
