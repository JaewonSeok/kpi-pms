...
async function main() {
  ...
  await run('activation readiness API and UI are read-only and admin-gated', () => {
    ...
    assert.equal(clientSource.includes('전용 평가 Workbench 열기'), true)
    assert.equal(clientSource.includes('실제 평가 흐름은 전용 Workbench에서 preview로 확인합니다.'), true)
    assert.equal(clientSource.includes('고급 진단 / Readiness 상세'), true)
    assert.equal(clientSource.includes('공식 전환 준비 / Dry-run 도구'), true)
    assert.equal(clientSource.includes('대표/최종 보고'), true)
    assert.equal(clientSource.includes('개발자/Watch-only'), true)
    assert.equal(clientSource.includes('공식 전환 준비 화면에서 상세 readiness를 확인하세요.'), true)
    assert.equal(clientSource.includes('공식 전환 준비 화면에서 dry-run/preflight 도구를 확인하세요.'), true)
    assert.equal(clientSource.includes('공식 전환 준비 화면에서 대표/최종 보고 readiness를 확인하세요.'), true)
    assert.equal(clientSource.includes('공식 전환 준비 화면에서 watch-only 항목과 금지 작업을 확인하세요.'), true)
    assert.equal(clientSource.includes('상세 readiness 열기'), true)
    assert.equal(clientSource.includes('dry-run 도구 열기'), true)
    assert.equal(clientSource.includes('대표/최종 보고 열기'), true)
    assert.equal(clientSource.includes('watch-only 열기'), true)
    assert.equal(clientSource.includes('전체 상세 진단 열기'), true)
    assert.equal(clientSource.includes('advanced collapsed'), true)
    ...
    assert.equal(clientSource.includes('용어 설명'), true)
    ...
    assert.equal(clientSource.includes('쓰기 없는 사전 실행 검토'), true)
    assert.equal(clientSource.includes('실제 반영'), true)
    assert.equal(clientSource.includes('기존 데이터를 평가 구조에 맞게 채우는 작업'), true)
    ...
    assert.equal(clientSource.includes('2026 평가 운영 대시보드'), true)
    assert.equal(clientSource.includes('2026 공식 전환 준비'), true)
    assert.equal(clientSource.includes('상세 공식 전환 진단과 dry-run 도구는 고급 전환 준비 화면에서 확인하세요.'), true)
    ...
    assert.equal(performancePageSource.includes('presentationMode="performance-dashboard"'), true)
    ...
    assert.equal(clientSource.includes('공식 readiness 대상 주기 지정'), true)
    ...
    assert.equal(navigationSource.includes('성과평가 운영'), true)
    assert.equal(navigationSource.includes('평가 Workbench'), true)
    assert.equal(navigationSource.includes('공식 전환 준비'), true)
