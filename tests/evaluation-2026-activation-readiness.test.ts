...
async function main() {
  ...
  await run('activation readiness API and UI are read-only and admin-gated', () => {
    ...
    assert.equal(clientSource.includes('관련 화면 바로가기'), true) // '오늘 할 일' → '관련 화면 바로가기' 개편(2026-07)
    assert.equal(clientSource.includes('평가 워크벤치 바로가기'), false) // 하단 패널 제거(2026-07) — 링크는 '관련 화면 바로가기'로 통합
    assert.equal(clientSource.includes('공식 전환 상태'), true)
    assert.equal(clientSource.includes('주요 병목 항목'), true) // 'Top 3 해소 필요 항목' → '주요 병목 항목' 개편(2026-07)
    assert.equal(clientSource.includes('전용 평가 워크벤치 열기'), true)
    assert.equal(clientSource.includes('실제 평가 흐름은 전용 평가 워크벤치에서 미리보기로 확인합니다.'), false) // 제거된 패널 설명(2026-07)
    ...
    assert.equal(clientSource.includes('href="/admin/evaluation-ops"'), false) // 사이드바 정리(2026-07)로 패널 링크 제거 — 재유입 방지 가드
    ...
    assert.equal(performancePageSource.includes('<PerformanceHrOpsDashboard data={data}') && performancePageSource.includes('canSeeAllInCycle'), true)
    ...
    assert.equal(navigationSource.includes('업적평가 모니터링'), false)
    ...
    assert.equal(navigationSource.includes('공식 전환 체크리스트'), false)
    assert.equal(navigationSource.includes('/admin/evaluation-readiness'), false)
