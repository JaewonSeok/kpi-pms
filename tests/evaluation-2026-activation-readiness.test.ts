...
async function main() {
  ...
  await run('activation readiness API and UI are read-only and admin-gated', () => {
    ...
    assert.equal(navigationSource.includes('팀원 KPI 검토'), true)
    assert.equal(navigationSource.includes('HR 평가 운영 대시보드'), true)
    assert.equal(navigationSource.includes('평가 워크벤치 미리보기'), true)
